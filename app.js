...
const builtins=[
  ex('smith_bench','Smith Bench Press','Smith','strength','heavy',['Chest'],['Triceps','Front Delts'],{weightSource:'barbell',e1rm:true}),
  ex('incline_smith','Incline Smith Bench','Smith','strength','heavy',['Chest'],['Triceps','Front Delts'],{weightSource:'barbell',e1rm:true}),
  ...
  ex('tib_bar','Tib Bar Raise','Tib bar','strength','small',['Tibialis'],[],{weightSource:'generic'}),
  ex('pullup','Pull-Up','Bodyweight','bodyweight','skill',['Lats'],['Upper Back','Biceps','Grip'],{special:['negatives','hold']}),
  ex('assisted_pullup','Assisted Pull-Up Machine','Machine','assisted','compound',['Lats'],['Upper Back','Biceps'],{}),
  ex('goblet_squat','Goblet Squat','Kettlebell','strength','compound',['Quads','Glutes'],['Core'],{weightSource:'kb'}),
  ex('barbell_squat','Barbell Squat','Barbell','strength','heavy',['Quads','Glutes'],['Hamstrings','Core'],{weightSource:'barbell',e1rm:true}),
  ex('smith_squat','Smith Squat','Smith','strength','heavy',['Quads','Glutes'],['Hamstrings','Core'],{weightSource:'barbell',e1rm:true}),
  ex('preacher_curl','2-Hand Preacher Curl','Bench / machine','strength','accessory',['Biceps'],['Forearms'],{weightSource:'machine'}),
  ...
  ex('calf_raise','Calf Raise','Bodyweight / vest','bodyweight','accessory',['Calves'],[],{}),
  ex('smith_calf_raise','Smith Calf Raise','Smith','strength','accessory',['Calves'],[],{weightSource:'barbell'}),
  ex('one_arm_bent_row','1-Arm Bent Row','DB / KB','strength','compound',['Lats','Upper Back'],['Biceps','Rear Delts','Grip'],{weightSource:'generic',perSide:true}),
  ex('kb_rdl','KB RDL','Kettlebell','strength','compound',['Hamstrings','Glutes'],['Back/Erectors','Grip'],{weightSource:'kb'}),
  ex('barbell_rdl','Barbell RDL','Barbell','strength','heavy',['Hamstrings','Glutes'],['Back/Erectors','Grip'],{weightSource:'barbell',e1rm:true}),
  ex('db_rdl','DB RDL','Dumbbell','strength','compound',['Hamstrings','Glutes'],['Back/Erectors','Grip'],{weightSource:'db'}),
  ex('dip_machine','Dip Machine','Machine','strength','compound',['Chest','Triceps'],['Front Delts'],{weightSource:'machine'}),
  ex('dips','Dips','Bodyweight','bodyweight','compound',['Chest','Triceps'],['Front Delts'],{}),
  ex('reverse_lunge','Reverse Lunge','DB / KB / bodyweight','strength','compound',['Quads','Glutes'],['Hamstrings','Core'],{weightSource:'generic',perSide:true}),
  ex('rear_delt_fly','Rear Delt Fly','Dumbbell / cable','strength','small',['Rear Delts'],['Upper Back'],{weightSource:'db'}),
  ex('lateral_raise','Lateral Raise','Dumbbell','strength','small',['Side Delts'],[],{weightSource:'db'}),
  ex('seated_leg_curl','Seated Leg Curl','Machine','strength','accessory',['Hamstrings'],[],{weightSource:'machine'}),
  ex('leg_extension','Leg Extension','Machine','strength','accessory',['Quads'],[],{weightSource:'machine'}),
  ...
  ex('plank','Plank','Bodyweight / vest','plank','core',['Core'],['Shoulders'],{}),
  ex('plank_pullthrough','Plank Pull-Through','KB / DB','plankpull','core',['Core'],['Shoulders','Lats'],{weightSource:'generic'}),
  ex('stepdown','Stepdown','Bodyweight / vest','stepdown','accessory',['Quads','Glutes'],['Calves'],{perSide:true}),
  ex('deadhang','Deadhang','Bodyweight','hold','skill',['Grip'],['Lats','Shoulders'],{}),
  ex('pushup','Push-Up','Bodyweight / vest','pushup','compound',['Chest'],['Triceps','Front Delts'],{}),
  ex('farmer_carry','Farmer Carry','KB / DB / vest','carry','compound',['Grip','Upper Back'],['Core','Traps'],{carryType:'farmer'}),
  ex('suitcase_carry','Suitcase Carry','KB / DB / vest','carry','compound',['Core','Grip'],['Obliques','Upper Back'],{carryType:'suitcase'}),
  ...
  ex('triceps_extension','Triceps Extension','Cable / DB','strength','accessory',['Triceps'],[],{weightSource:'generic'}),
  ex('hanging_knee_raise','Hanging Knee Raise','Bodyweight','bodyweight','accessory',['Core'],['Hip Flexors','Grip'],{}),
  ex('shrugs','Shrugs','Barbell / DB / Smith','strength','accessory',['Traps'],['Grip'],{weightSource:'generic'}),
...
const defaultTemplates={
  A:{id:'A',name:'Workout A',supersets:[{id:'A_ss1',name:'Superset 1',exerciseIds:['smith_bench','tib_bar','pullup','goblet_squat','deadhang','stepdown']}],favorites:['seated_leg_curl','dip_machine','one_arm_bent_row','kb_rdl','calf_raise','preacher_curl','lateral_raise','rear_delt_fly','leg_extension','plank','treadmill_walk'],rest:['barbell_rdl','reverse_lunge','triceps_extension','pushup','farmer_carry','sled']},
  B:{id:'B',name:'Workout B',supersets:[{id:'B_ss1',name:'Superset 1',exerciseIds:['incline_smith','pullup','tib_bar','reverse_lunge','deadhang','leg_extension']}],favorites:['smith_bench','pushup','one_arm_bent_row','triceps_extension','seated_leg_curl','goblet_squat','barbell_rdl','stepdown','calf_raise','preacher_curl','lateral_raise','rear_delt_fly','plank','treadmill_walk'],rest:['dip_machine','kb_rdl','farmer_carry','suitcase_carry','sled']}
};
...
function loadState(){
  let raw=null;try{raw=JSON.parse(localStorage.getItem(STORE_KEY)||'null');}catch{}
...
function migrateState(old){
  ...
  s.templates=migrateTemplates(old.templates||{});
...
function migrateEntry(z){
  ...
  return {id:z.id||uid('set'),exerciseId:exId,timestamp:z.timestamp||nowIso(),mode:z.mode||exercise(exId)?.mode||'strength',setType:z.setType||'Working',rir:z.rir??'',quickNote:z.quickNote||'',note:z.note||'',weight:number(z.weight),reps:number(z.reps),negatives:number(z.negatives),partials:number(z.partials),holdSeconds:number(z.holdSeconds),assist:number(z.assist),...clone(z),exerciseId:exId};
}
...
function migrateActive(a){
  if(a.layout)return {...a,sets:(a.sets||[]).map(migrateEntry)};
  ...
  return {id:a.id||uid('wo'),template:a.template||'',title:a.title||'Workout',startedAt:a.startedAt||nowIso(),sessionFocus:a.sessionFocus||'',notes:a.notes||'',sets:(a.sets||[]).map(migrateEntry),layout:{supersets:ss.exerciseIds.length?[ss]:[],favorites,rest,done,homeSections,lastSections:{}}};
}
...
function startWorkout(templateId){
  ...
  state.active={id:uid('wo'),template:templateId,title:isBlank?'Blank Workout':t.name,startedAt:nowIso(),endedAt:null,sessionFocus:'',notes:'',sets:[],layout:templateToLayout(t)};
...
function addActiveExercise(id,target='rest'){if(allActiveIds().includes(id))return;state.active.layout.homeSections[id]=target==='favorites'?'favorites':'rest';state.active.layout[target==='favorites'?'favorites':'rest'].push(id);saveState();renderWorkout();}
...
function exerciseCard(id,section){
  const e=exercise(id);if(!e)return'';const last=lastActiveEntry(id);const rec=recoveryStatus(id);const inDone=section==='done',inSS=section.startsWith('ss:');const starred=state.active.layout.homeSections[id]==='favorites';
  return `<div class="exercise-card" data-exercise-id="${id}"><div class="drag-handle" data-drag-handle title="Drag inside this section">≡</div><div><div class="exercise-name">${esc(e.name)}</div><div class="exercise-meta">${esc(e.equipment)} • ${restLabel(e)}</div>${last?`<div class="last-line">${esc(formatEntry(last,e))}</div>`:''}${!inDone?`<div class="recovery ${rec.wait?'wait':''}" data-recovery-id="${id}">${esc(rec.text)}</div>`:''}</div><div class="card-actions">${inDone?`<button class="mini-btn" data-reactivate="${id}">Reactivate</button>`:`<button class="log-btn" data-log="${id}">LOG</button><button class="mini-btn ${starred?'active':''}" data-favorite="${id}" title="Favorite">${starred?'★':'☆'}</button>${inSS?`<button class="mini-btn" data-remove-ss="${id}">−SS</button><button class="mini-btn" data-move-ss="${id}">Move</button>`:`<button class="mini-btn" data-move-ss="${id}">+SS</button>`}<button class="mini-btn" data-done="${id}">Done</button>`}</div></div>`;
}
...
function renderLoggerFields(existing=null){
  const e=exercise(logger.exerciseId);if(!e)return;document.getElementById('sheetTitle').textContent=e.name;const seed=existing||lastActiveEntry(e.id)||previousWorkoutEntry(e.id)||{};const f=document.getElementById('logFields');let html='';
  switch(e.mode){
    case 'strength':{
...
document.addEventListener('click',ev=>{
  ...
  if(b.dataset.editHistoryEntry){const w=state.workouts.find(w=>w.id===b.dataset.workoutId),s=w?.sets.find(s=>s.id===b.dataset.editHistoryEntry);if(s)openLogger(s.exerciseId,{scope:'history',workoutId:w.id,entryId:s.id});return;}
