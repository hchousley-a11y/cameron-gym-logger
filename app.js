...
const SET_TYPES=['Warmup','Working','Top Set','Backoff','AMRAP','Failure','Slow Eccentric','Technique','Hard','Barely','Drop Set','Myo'];
const LIMITERS=['Strength','Local Muscle Fatigue','Cardio / Breathing','Grip','Technique Failure','Tempo Loss','ROM Loss','Joint Discomfort','Pain','Balance / Stability','Mobility / Position','Other'];
const RPE_VALUES=['5','6','7','8','9','10'];
const REP_VALUES=Array.from({length:101},(_,i)=>i);
const PULLUP_TYPES=['Full','Partial'];
const TREAD_SPEEDS=Array.from({length:201},(_,i)=>(i/10).toFixed(1));
const TREAD_INCLINES=Array.from({length:181},(_,i)=>(-3+i/10).toFixed(1));
...
const builtins=[
  ...
  ex('pullup','Pull-Up','Bodyweight','bodyweight','skill',['Lats'],['Upper Back','Biceps','Grip'],{special:['negatives','hold','partial'],restOverride:180}),
  ...
  ex('rear_delt_fly','Rear Delt Fly','Dumbbell / cable','strength','small',['Rear Delts'],['Upper Back'],{weightSource:'rear_delt',restOverride:60}),
...
let logger={exerciseId:null,editScope:null,workoutId:null,entryId:null,setTypes:['Working'],quickNote:'',limiters:[]};
...
function migrateEntry(z){
  ...
  if(!Array.isArray(out.setTypes)||!out.setTypes.length)out.setTypes=[out.setType||'Working'];
  return out;
}
...
function workingEntries(exId){return (state.active?.sets||[]).filter(s=>s.exerciseId===exId&&!entrySetTypes(s).includes('Warmup'));}
...
function weightOptions(source='generic',max=500){
  ...
  if(source==='rear_delt')return [...Array.from({length:9},(_,i)=>i*2.5),...Array.from({length:17},(_,i)=>25+i*5)];
  ...
  if(source==='barbell'){const a=[];for(let x=45;x<=max;x+=5)a.push(x);return a;}
...
function entrySetTypes(s){if(Array.isArray(s?.setTypes)&&s.setTypes.length)return s.setTypes;return [s?.setType||'Working'];}
function repsHtml(id,label,value=10){return selectHtml(id,label,REP_VALUES,Math.max(0,Math.min(100,Math.round(number(value)))));}
function rpeHtml(value='7'){return selectHtml('fRpe','RPE',RPE_VALUES,String(value||'7'));}
function muscleTotalsForWorkout(w){const m={};(w?.sets||[]).forEach(s=>{if(entrySetTypes(s).includes('Warmup'))return;const e=exercise(s.exerciseId);if(!e)return;(e.primary||[]).forEach(x=>m[x]=(m[x]||0)+1);(e.secondary||[]).forEach(x=>m[x]=(m[x]||0)+.5);});return Object.entries(m).sort((a,b)=>b[1]-a[1]);}
function comparableWorkout(exId){if(!state.active)return null;const ws=[...state.workouts].sort((a,b)=>new Date(b.startedAt)-new Date(a.startedAt));const sameTemplate=state.active.template&&state.active.template!=='blank';return ws.find(w=>w.sets.some(s=>s.exerciseId===exId)&&(!sameTemplate||w.template===state.active.template))||null;}
function renderPreviousComparable(){const box=document.getElementById('previousComparable');if(!box)return;if(!state.active||logger.editScope==='history'){box.classList.add('hidden');return;}const w=comparableWorkout(logger.exerciseId);if(!w){box.classList.remove('hidden');box.innerHTML='<strong>Previous comparable workout</strong><div class="subtle">No previous comparable sets.</div>';return;}const sets=w.sets.filter(s=>s.exerciseId===logger.exerciseId).sort((a,b)=>new Date(a.timestamp)-new Date(b.timestamp));box.classList.remove('hidden');box.innerHTML=`<strong>LAST ${state.active.template==='blank'?'OCCURRENCE':esc(state.active.title)} • ${fmtDate(w.startedAt)}</strong><div class="previous-set-list">${sets.map(s=>`<div>${esc(formatEntry(s))}</div>`).join('')}</div>`;}
...
function openLogger(exId,edit=null){
  const e=exercise(exId);if(!e)return;logger={exerciseId:exId,editScope:edit?.scope||null,workoutId:edit?.workoutId||null,entryId:edit?.entryId||null,setTypes:['Working'],quickNote:'',limiters:[]};
  const existing=entryByEditRef();if(existing){logger.exerciseId=existing.exerciseId;logger.setTypes=entrySetTypes(existing);logger.quickNote=existing.quickNote||'';logger.limiters=existing.limiters||[];}
  ...
  renderLoggerFields(existing);renderPreviousComparable();renderTodayEntries();document.getElementById('deleteEntryBtn').classList.toggle('hidden',!existing);document.getElementById('saveEntryBtn').textContent=existing?'SAVE CHANGES ✓':'LOG ✓';document.getElementById('saveEntryTopBtn').textContent=existing?'SAVE CHANGES ✓':'LOG SET ✓';openSheet('logSheet');
}
function renderLoggerFields(existing=null){
  ...
  switch(e.mode){
    case 'strength':{
      const ws=weightOptions(e.weightSource||'generic');html+=`<div class="field-grid">${selectHtml('fWeight','Weight (lb)',ws,seed.weight??ws[0])}${repsHtml('fReps',e.perSide?'Reps / side':'Reps',seed.reps??10)}${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div>`;
      break;
    }
    case 'bodyweight':{
      html+=`<div class="field-grid">${selectHtml('fWeight','Added lb',weightOptions('bodyweight'),seed.weight??0)}${repsHtml('fReps',e.perSide?'Reps / side':'Reps',seed.reps??5)}${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div>`;
      if(e.special?.includes('negatives')||e.id==='pullup')html+=`<div class="field-grid three">${e.id==='pullup'?selectHtml('fPullupType','Rep type',PULLUP_TYPES,seed.pullupType||'Full'):''}${numberHtml('fNegatives','Negatives',seed.negatives??0,1,0)}${numberHtml('fHold','Top hold sec',seed.holdSeconds??0,1,0)}</div>`;
      break;
    }
    case 'assisted':html+=`<div class="field-grid">${selectHtml('fAssist','Assistance lb',Array.from({length:61},(_,i)=>i*5),seed.assist??70)}${repsHtml('fReps','Reps',seed.reps??8)}${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div>`;break;
    case 'hold':html+=`<div class="field-grid">${selectHtml('fWeight','Added lb',weightOptions('bodyweight'),seed.weight??0)}${durationFields(seed.durationSeconds||seed.holdSeconds||30)}${rpeHtml(seed.rpe||'7')}</div>`;break;
    case 'plank':html+=`<div class="field-grid">${selectHtml('fWeight','Added lb',VEST_WEIGHTS,seed.weight??0)}${durationFields(seed.durationSeconds||seed.holdSeconds||60)}${rpeHtml(seed.rpe||'7')}</div>`;break;
    case 'plankpull':html+=`<div class="field-grid">${selectHtml('fWeight','Weight lb',weightOptions(e.weightSource||'generic'),seed.weight??25)}${repsHtml('fReps','Pull-through reps',seed.reps??10)}${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div><div class="field-grid two">${durationFields(seed.durationSeconds||60,'up')}</div>`;break;
    case 'stepdown':html+=`<div class="field-grid four">${selectHtml('fWeight','Added lb',VEST_WEIGHTS,seed.weight??0)}${repsHtml('fReps','Reps / side',seed.reps??10)}${selectHtml('fStepHeight','Step height',STEP_HEIGHTS,seed.stepHeight||'Treadmill')}${rpeHtml(seed.rpe||'7')}</div>`;break;
    case 'pushup':html+=`<div class="field-grid four">${selectHtml('fWeight','Added lb',VEST_WEIGHTS,seed.weight??0)}${repsHtml('fReps','Reps',seed.reps??10)}${selectHtml('fStyle','Style',PUSH_STYLES,seed.style||'Normal')}${selectHtml('fSetup','Setup',PUSH_SETUPS,seed.setup||'Flat')}</div><div class="field-grid two">${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div>`;break;
    ...
    default:html+=`<div class="field-grid">${numberHtml('fWeight','Weight',seed.weight??0,5,0)}${repsHtml('fReps','Reps',seed.reps??10)}${selectHtml('fRir','RIR',RIR_VALUES,seed.rir||'2')}</div>`;
  }
...
function carryFields(e,seed){
  ...
  return `<div class="field-grid four">${selectHtml('fLoadType','Load type',['KB','DB','Vest'],loadType)}${selectHtml('fCarryWeight',e.carryType==='farmer'?'Weight / hand':'Carry weight',weights,seed.carryWeight??weights[0])}${selectHtml('fVest','Vest add-on',VEST_WEIGHTS,seed.vestWeight??0)}${numberHtml('fDistance','Distance ft',seed.distanceFt??100,1,0)}</div><div class="field-grid three">${durationFields(seed.durationSeconds||0)}${e.carryType==='suitcase'?selectHtml('fSide','Side',['Left','Right','Both sequential'],seed.side||'Both sequential'):''}${rpeHtml(seed.rpe||'7')}</div>`;
}
function treadmillFields(e,seed){return `<div class="field-grid four">${selectHtml('fSpeed','Speed mph',TREAD_SPEEDS,Number(seed.speed??3).toFixed(1))}${selectHtml('fIncline','Incline %',TREAD_INCLINES,Number(seed.incline??0).toFixed(1))}${durationFields(seed.durationSeconds||1200)}${selectHtml('fVest','Vest lb',VEST_WEIGHTS,seed.vestWeight??0)}</div><div class="field-grid four">${numberHtml('fDistanceMiles','Distance mi',seed.distanceMiles??'',0.01,0,'auto')}${numberHtml('fCalories','Calories',seed.calories??'',1,0,'optional')}${numberHtml('fAvgHr','Avg HR',seed.avgHr??'',1,0,'optional')}${numberHtml('fMaxHr','Max HR',seed.maxHr??'',1,0,'optional')}</div>`;}
function runWalkFields(seed){return `<div class="panel"><strong>Run segment</strong><div class="field-grid four">${selectHtml('fRunSpeed','Run mph',TREAD_SPEEDS,Number(seed.runSpeed??6.2).toFixed(1))}${selectHtml('fRunIncline','Incline %',TREAD_INCLINES,Number(seed.runIncline??0).toFixed(1))}${durationFields(seed.runSeconds||90,'run')}</div></div><div class="panel"><strong>Walk segment</strong><div class="field-grid four">${selectHtml('fWalkSpeed','Walk mph',TREAD_SPEEDS,Number(seed.walkSpeed??3).toFixed(1))}${selectHtml('fWalkIncline','Incline %',TREAD_INCLINES,Number(seed.walkIncline??0).toFixed(1))}${durationFields(seed.walkSeconds||90,'walk')}</div></div><div class="field-grid four">${numberHtml('fCycles','Cycles',seed.cycles??10,1,1)}${selectHtml('fVest','Vest lb',VEST_WEIGHTS,seed.vestWeight??0)}${numberHtml('fDistanceMiles','Distance mi',seed.distanceMiles??'',0.01,0,'auto')}${numberHtml('fCalories','Calories',seed.calories??'',1,0,'optional')}</div>`;}
function renderCommonFields(existing){
  logger.setTypes=existing?entrySetTypes(existing):(logger.setTypes?.length?logger.setTypes:['Working']);logger.quickNote=existing?.quickNote||logger.quickNote||'';logger.limiters=existing?.limiters||logger.limiters||[];
  document.getElementById('setTypeTags').innerHTML=SET_TYPES.map(x=>`<button class="tag-btn ${logger.setTypes.includes(x)?'active':''}" data-set-type="${x}">${x}</button>`).join('');
  document.getElementById('quickNotes').innerHTML=QUICK_NOTES.map(x=>`<button class="tag-btn ${x===logger.quickNote?'active':''}" data-quick-note="${x}">${x}</button>`).join('');
  document.getElementById('limiterTags').innerHTML=LIMITERS.map(x=>`<button class="tag-btn ${logger.limiters.includes(x)?'active':''}" data-limiter="${x}">${x}</button>`).join('');
  document.getElementById('limiterExtraInput').value=existing?.limiterNote||'';document.getElementById('limiterExtraWrap').classList.toggle('hidden',!logger.limiters.some(x=>x==='Other'||x==='Pain'));
  document.getElementById('setNoteInput').value=existing?.note||'';
}
...
function readLoggerEntry(){
  const e=exercise(logger.exerciseId),base=entryByEditRef()||{};const out={...base,id:base.id||uid('set'),exerciseId:e.id,mode:e.mode,timestamp:base.timestamp||nowIso(),setType:(logger.setTypes?.[0]||'Working'),setTypes:logger.setTypes||['Working'],rir:document.getElementById('fRir')?.value||'',rpe:document.getElementById('fRpe')?.value||'',quickNote:logger.quickNote||'',limiters:logger.limiters||[],limiterNote:document.getElementById('limiterExtraInput')?.value.trim()||'',note:document.getElementById('setNoteInput').value.trim()};
  ...
  switch(e.mode){
    ...
    case'bodyweight':out.weight=val('fWeight');out.reps=val('fReps');out.negatives=val('fNegatives');out.holdSeconds=val('fHold');if(document.getElementById('fPullupType'))out.pullupType=document.getElementById('fPullupType').value;break;
...
function formatEntry(s,e=exercise(s.exerciseId)){
  if(!e)return'Unknown entry';let t='';const rir=s.rir?` @${s.rir}`:(s.rpe?` @RPE ${s.rpe}`:'');
  switch(e.mode){
  ...
  const types=entrySetTypes(s).filter(x=>x!=='Working');if(types.length)t+=` • ${types.join(' + ')}`;if(s.pullupType==='Partial')t+=' • Partial';if(s.quickNote)t+=` • ${s.quickNote}`;if(s.limiters?.length)t+=` • Limited: ${s.limiters.join(' + ')}`;return t;
}
...
function exportCsv(){const head=['workout_id','workout_title','workout_start','workout_end','session_focus','workout_notes','entry_timestamp','exercise','equipment','mode','set_type','set_types','rir','rpe','quick_note','limiters','limiter_note','entry_note','weight_lb','reps','negatives','hold_seconds','assist_lb','step_height','style','setup','load_type','carry_weight_lb','vest_lb','distance_ft','duration_seconds','speed_mph','incline_percent','distance_miles','calories','avg_hr','max_hr','run_speed','run_seconds','walk_speed','walk_seconds','cycles','sled_direction','primary_muscles','secondary_muscles'];const rows=[head];state.workouts.forEach(w=>w.sets.forEach(s=>{const e=exercise(s.exerciseId);rows.push([w.id,w.title,w.startedAt,w.endedAt,w.sessionFocus||'',w.notes||'',s.timestamp,e?.name||s.exerciseId,e?.equipment||'',e?.mode||'',s.setType||'',entrySetTypes(s).join('|'),s.rir||'',s.rpe||'',s.quickNote||'',(s.limiters||[]).join('|'),s.limiterNote||'',s.note||'',s.weight||'',s.reps||'',s.negatives||'',s.holdSeconds||s.durationSeconds||'',s.assist||'',s.stepHeight||'',s.style||'',s.setup||'',s.loadType||'',s.carryWeight||'',s.vestWeight||'',s.distanceFt||'',s.durationSeconds||'',s.speed||'',s.incline||'',s.distanceMiles||'',s.calories||'',s.avgHr||'',s.maxHr||'',s.runSpeed||'',s.runSeconds||'',s.walkSpeed||'',s.walkSeconds||'',s.cycles||'',s.direction||'',(e?.primary||[]).join('|'),(e?.secondary||[]).join('|')]);}));download(`farmer-jim-v040-${new Date().toISOString().slice(0,10)}.csv`,rows.map(r=>r.map(csvEscape).join(',')).join('\n'),'text/csv');}
...
document.addEventListener('click',ev=>{
  ...
  if(b.dataset.setType){const x=b.dataset.setType;if(x==='Warmup')logger.setTypes=['Warmup'];else{logger.setTypes=(logger.setTypes||[]).filter(v=>v!=='Warmup');if(logger.setTypes.includes(x))logger.setTypes=logger.setTypes.filter(v=>v!==x);else if(logger.setTypes.length<3)logger.setTypes.push(x);if(!logger.setTypes.length)logger.setTypes=['Working'];}document.querySelectorAll('[data-set-type]').forEach(el=>el.classList.toggle('active',logger.setTypes.includes(el.dataset.setType)));return;}
  if(b.dataset.limiter){const x=b.dataset.limiter;logger.limiters=logger.limiters||[];logger.limiters=logger.limiters.includes(x)?logger.limiters.filter(v=>v!==x):[...logger.limiters,x];document.querySelectorAll('[data-limiter]').forEach(el=>el.classList.toggle('active',logger.limiters.includes(el.dataset.limiter)));document.getElementById('limiterExtraWrap').classList.toggle('hidden',!logger.limiters.some(v=>v==='Other'||v==='Pain'));return;}
  ...
  if(b.id==='saveEntryBtn'||b.id==='saveEntryTopBtn'){saveLoggerEntry();return;}
  ...
  if(b.id==='deleteWorkoutBtn'){const w=state.workouts.find(w=>w.id===currentHistoryWorkoutId);if(w&&confirm(`Delete entire workout: ${w.title} — ${fmtDate(w.startedAt)}? This cannot be undone.`)){state.workouts=state.workouts.filter(x=>x.id!==w.id);saveState();closeSheets();renderHistory();renderHome();toast('Workout deleted');}return;}
